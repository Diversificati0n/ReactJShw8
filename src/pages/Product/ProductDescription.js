import React from 'react';

const ProductDescription = () => {
    return (
        <div className="product-description__box">
            <div className="product-description">
                <h3 className="product-description__title">WOMEN COLLECTION</h3>
                <svg width="63.000000" height="3.025391" viewBox="0 0 63 3.02539" fill="none">
                    <defs />
                    <path id="Rectangle 28" d="M0 0L63 0L63 3.02539L0 3.02539L0 0Z" fill="#EF5B70" fillOpacity="1.000000"
                        fillRule="evenodd" />
                </svg>
                <h3 className="description__title">MOSCHINO CHEAP AND CHIC</h3>
                <p className="product-description__text">Compellingly actualize fully researched processes before proactive
                    outsourcing. Progressively syndicate collaborative architectures before cutting-edge services.
                    Completely
                    visualize parallel core competencies rather than exceptional portals. </p>
                <p className="product-description__price">$561</p>
                <svg className="product-description__svg" width="642.000000" height="1.000000" viewBox="0 0 642 1" fill="none">
                    <defs />
                    <path id="Vector" d="M641.5 0.5L0.5 0.5L641.5 0.5Z" stroke="#EAEAEA" strokeOpacity="1.000000"
                        strokeWidth="1.000000" strokeLinejoin="round" />
                </svg>
                <div className="description__link">
                    <p className="product-description__link">CHOOSE COLOR <svg width="10.020020" height="5.046143"
                            viewBox="0 0 10.02 5.04614" fill="none">
                            <defs />
                            <path id="Vector"
                                d="M5.00195 5.04443C4.83545 5.04468 4.67334 4.98608 4.54492 4.87866L0.258301 1.27612C0.112305 1.15381 0.0205078 0.978271 0.00292969 0.787598C-0.0141602 0.597168 0.0439453 0.407471 0.165039 0.260254C0.286621 0.113037 0.460938 0.0205078 0.649902 0.00292969C0.838379 -0.0144043 1.02686 0.0444336 1.17285 0.166504L5.00195 3.39453L8.83154 0.281982C8.90479 0.222168 8.98877 0.177246 9.0791 0.150391C9.16943 0.123535 9.26416 0.114746 9.35742 0.124756C9.45117 0.134766 9.54199 0.16333 9.62451 0.208984C9.70703 0.254395 9.78027 0.315918 9.83887 0.389893C9.9043 0.464111 9.9541 0.551025 9.98486 0.645264C10.0151 0.739502 10.0264 0.839111 10.0166 0.937988C10.0073 1.03662 9.97803 1.13257 9.93018 1.21924C9.88232 1.30591 9.81738 1.38184 9.73926 1.44189L5.45215 4.92188C5.31982 5.01245 5.16162 5.05542 5.00195 5.04443Z"
                                fill="#6F6E6E" fillOpacity="1.000000" fillRule="nonzero" />
                        </svg>
                    </p>
                    <p className="product-description__link">CHOOSE SIZE <svg width="10.020020" height="5.046143"
                            viewBox="0 0 10.02 5.04614" fill="none">
                            <defs />
                            <path id="Vector"
                                d="M5.00195 5.04443C4.83545 5.04468 4.67334 4.98608 4.54492 4.87866L0.258301 1.27612C0.112305 1.15381 0.0205078 0.978271 0.00292969 0.787598C-0.0141602 0.597168 0.0439453 0.407471 0.165039 0.260254C0.286621 0.113037 0.460938 0.0205078 0.649902 0.00292969C0.838379 -0.0144043 1.02686 0.0444336 1.17285 0.166504L5.00195 3.39453L8.83154 0.281982C8.90479 0.222168 8.98877 0.177246 9.0791 0.150391C9.16943 0.123535 9.26416 0.114746 9.35742 0.124756C9.45117 0.134766 9.54199 0.16333 9.62451 0.208984C9.70703 0.254395 9.78027 0.315918 9.83887 0.389893C9.9043 0.464111 9.9541 0.551025 9.98486 0.645264C10.0151 0.739502 10.0264 0.839111 10.0166 0.937988C10.0073 1.03662 9.97803 1.13257 9.93018 1.21924C9.88232 1.30591 9.81738 1.38184 9.73926 1.44189L5.45215 4.92188C5.31982 5.01245 5.16162 5.05542 5.00195 5.04443Z"
                                fill="#6F6E6E" fillOpacity="1.000000" fillRule="nonzero" />
                        </svg>
                    </p>
                </div>
                <button className="product-description__button">ADD TO CART</button>
            </div>
        </div>
    );
}

export default ProductDescription;